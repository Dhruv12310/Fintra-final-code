      ...
      >
        <div className="flex items-center gap-2">
          {hasResult ? (
            ...
            <Loader2 className="w-3 h-3 flex-shrink-0 animate-spin" style={{ color: 'var(--neon-cyan)' }} />
          )}
          <Wrench className="w-3 h-3 flex-shrink-0" style={{ color: 'var(--neon-cyan)' }} />
          <span className="font-medium" style={{ color: 'var(--neon-cyan)' }}>{toolLabel}</span>
          {msg.collapsed ? <ChevronRight className="w-3 h-3 ml-auto" /> : <ChevronDown className="w-3 h-3 ml-auto" />}
        </div>

        {!msg.collapsed && (
          <div className="mt-2 space-y-1.5" style={{ borderTop: '1px solid rgba(34,211,238,0.1)', paddingTop: 8 }}>
  ...
  return (
    <div className="mt-3 rounded-lg overflow-hidden" style={{ border: '1px solid var(--border-color)' }}>
      ...
      {preview.unusual_patterns?.length > 0 && (
        <div
          className="px-3 py-2 space-y-0.5"
          style={{ borderTop: '1px solid var(--border-color)', background: 'rgba(217,70,239,0.04)' }}
        >
          {preview.unusual_patterns.map((p, i) => (
            <p key={i} className="text-xs" style={{ color: 'var(--neon-fuchsia)' }}>🔍 {p}</p>
          ))}
        </div>
      )}
    </div>
      ...
      >
        ...
        >
          {msg.status === 'approved' ? (
            ...
            <AlertCircle className="w-4 h-4" style={{ color: 'var(--neon-fuchsia)' }} />
          )}
  ...
  return (
    <div className={`flex mb-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        ...
        style={{
          background: isUser
            ? 'linear-gradient(135deg, rgba(217,70,239,0.25), rgba(34,211,238,0.2))'
            : 'var(--bg-card)',
          border: isUser
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {isEmpty && (
          <div className="flex flex-col items-center justify-center h-full gap-6 pb-8">
            <div>
              ...
              >
                <Sparkles className="w-7 h-7" style={{ color: 'var(--neon-fuchsia)' }} />
              </div>
          ...
          <div className="flex justify-start mb-2">
            ...
            >
              ...
              <span
                ...
                style={{ background: 'var(--neon-fuchsia)', verticalAlign: 'middle' }}
              />
            </div>
          </div>
        )}
        ...
        {streaming && !currentText && (
          <div className="flex justify-start mb-2">
            ...
            >
              <Loader2 className="w-3.5 h-3.5 animate-spin" style={{ color: 'var(--neon-fuchsia)' }} />
          ...
          <button
            ...
            style={{
              background: streaming || !input.trim()
                ? 'var(--bg-secondary)'
                : 'linear-gradient(135deg, var(--neon-fuchsia), var(--neon-cyan))',

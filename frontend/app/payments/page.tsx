...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, { bg: string; color: string }> = {
    ...
    partial: { bg: 'rgba(34,211,238,0.15)', color: 'var(--neon-cyan)' },
  ...
  return (
    <div className="min-h-screen p-8 space-y-6" style={{ color: 'var(--text-primary)' }}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em]" style={{ color: 'var(--text-muted)' }}>Receivables</p>
          <h1 className="text-3xl font-semibold mt-1">Payments</h1>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Customer payments and applications</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={card.label} className="rounded-2xl p-5" style={cardStyle}>

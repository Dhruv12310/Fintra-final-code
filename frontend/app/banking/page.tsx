              ...
              <div key={type}>
                ...
                {accounts.map(a => (
                  <button
                    ...
                    style={{ color: a.id === value ? 'var(--accent)' : 'var(--text-primary)', background: a.id === value ? 'var(--accent-subtle)' : 'transparent' }}
                    onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-secondary)')}
                    onMouseLeave={e => (e.currentTarget.style.background = a.id === value ? 'var(--accent-subtle)' : 'transparent')}
                  >
    ...
    <div className="flex flex-col min-h-screen" style={{ color: 'var(--text-primary)' }}>
      ...
      <div className="p-6 space-y-5">
        ...
        <PageHeader
          ...
          actions={
            <button onClick={fetchLinkToken} disabled={linkLoading} className="btn btn-primary">
          ...
          <div className="relative">
            {accounts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 rounded-xl" style={{ border: '2px dashed var(--border-color)' }}>
                ...
                <button onClick={fetchLinkToken} disabled={linkLoading} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg disabled:opacity-60" style={{ background: 'var(--accent)', color: '#fff' }}>
              ...
              {(['pending', 'posted', 'excluded'] as const).map((t, i) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      borderBottom: tab === t ? '2px solid var(--accent)' : '2px solid transparent', background: 'var(--bg-card)',

  ...
  return (
    <div className="min-h-screen p-6 space-y-6" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--text-muted)' }}>Accounts Payable</p>
          <h1 className="text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>Bills</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>Enter bills from vendors and track what you owe</p>
        </div>
        {!showCreateForm && (
          <button
      ...
      {showCreateForm && (
        <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
          <div className="px-8 pt-6 pb-5 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border-color)' }}>
            <h2 className="text-xl font-black tracking-widest" style={{ color: 'var(--neon-fuchsia)' }}>BILL</h2>
          ...
          <div className="px-8 py-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={resetForm} className="px-4 py-2 rounded-xl text-sm font-medium transition hover:opacity-80"
                style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-secondary)' }}>
                Cancel
              </button>
      ...
      {!showCreateForm && bills.length > 0 && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            { label: 'Total Outstanding', value: summaryStats.outstanding, icon: DollarSign, color: 'var(--neon-fuchsia)' },
            ...
            { label: 'Avg Bill Value', value: summaryStats.avg, icon: BarChart2, color: 'var(--neon-cyan)' },
          ].map(stat => (
      ...
      <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
          ...
          {[
            ...
            return (
              <button key={tab.key} onClick={() => { setStatusFilter(tab.key); setPage(1) }}
                ...
                style={{ color: active ? 'var(--neon-fuchsia)' : 'var(--text-muted)', borderBottom: active ? '2px solid var(--neon-fuchsia)' : '2px solid transparent', marginBottom: '-1px' }}>
                {tab.label}
                {count > 0 && (
                  <span className="ml-1.5 px-1.5 py-0.5 rounded-full text-xs font-semibold"
                    style={{ backgroundColor: active ? 'rgba(217,70,239,0.15)' : 'rgba(255,255,255,0.06)', color: active ? 'var(--neon-fuchsia)' : 'var(--text-muted)' }}>
                    {count}
                  </span>
                )}
              </button>
        ...
        <div className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: '1px solid var(--border-color)' }}>
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              className="w-full pl-9 pr-3 py-2 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-400/30"
              style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }}
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-3 px-4 py-2.5"
            style={{ backgroundColor: 'rgba(217,70,239,0.05)', borderBottom: '1px solid var(--border-color)' }}>
            <span className="text-sm font-semibold" style={{ color: 'var(--neon-fuchsia)' }}>{selectedIds.size} selected</span>
            ...
            <button onClick={() => setSelectedIds(new Set())} className="ml-auto px-3 py-1 rounded-lg text-xs font-medium transition hover:opacity-70"
          ...
          <>
            ...
            <div className="grid items-center px-4 py-2.5 text-xs font-semibold uppercase tracking-wider"
              ...
              <div className="flex justify-center">
                <input type="checkbox" className="w-3.5 h-3.5 cursor-pointer accent-fuchsia-400" checked={allPageSelected} onChange={toggleAll} />
              </div>
            ...
            {pagedBills.map(bill => {
              ...
              return (
                <div key={bill.id}>
                  <div
                    className="grid items-center px-4 py-3.5 cursor-pointer transition hover:bg-[rgba(217,70,239,0.02)]"
                    style={{ gridTemplateColumns: '36px 1fr 1.5fr 100px 100px 110px 110px 90px 48px', borderBottom: '1px solid var(--border-color)', backgroundColor: selected ? 'rgba(217,70,239,0.04)' : undefined }}
                    onClick={() => toggleExpand(bill.id)}
                  >
                    <div className="flex justify-center" onClick={e => { e.stopPropagation(); toggleOne(bill.id) }}>
                      <input type="checkbox" className="w-3.5 h-3.5 cursor-pointer accent-fuchsia-400" checked={selected} onChange={() => toggleOne(bill.id)} />
                    </div>
                    <span className="text-sm font-semibold" style={{ color: 'var(--neon-fuchsia)' }}>{bill.bill_number}</span>
                  ...
                  {expandedId === bill.id && (
                    <div className="px-6 pb-6 pt-4 space-y-4"
                      style={{ borderBottom: '1px solid var(--border-color)', backgroundColor: 'rgba(217,70,239,0.02)' }}>
                      {!expandedDetail ? (
                        <div className="flex justify-center py-6">
                          <Loader2 className="w-5 h-5 animate-spin" style={{ color: 'var(--neon-fuchsia)' }} />
                        </div>
                      ) : (
                        <>
                          <div>
                          ...
                          <div className="flex gap-2 pt-1">
                            {bill.status === 'draft' && (
                              <button onClick={e => { e.stopPropagation(); handlePostBill(bill.id) }}
                                ...
                                style={{ background: 'linear-gradient(135deg, var(--neon-emerald), var(--neon-fuchsia))' }}>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: Math.min(pageCount, 7) }, (_, i) => i + 1).map(p => (
                    <button key={p} onClick={() => setPage(p)} className="w-8 h-8 rounded-xl text-xs font-medium transition"
                      style={{ backgroundColor: page === p ? 'rgba(217,70,239,0.15)' : 'var(--bg-primary)', color: page === p ? 'var(--neon-fuchsia)' : 'var(--text-secondary)', border: page === p ? '1px solid rgba(217,70,239,0.3)' : '1px solid var(--border-color)' }}>
                      {p}
                    </button>
                  ))}

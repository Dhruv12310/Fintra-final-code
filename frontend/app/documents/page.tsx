...
const DOC_TYPE_LABELS: Record<string, { label: string; color: string }> = {
  bill: { label: 'Bill / Supplier Invoice', color: 'var(--accent)' },
  invoice: { label: 'Sales Invoice', color: 'var(--accent)' },
  ...
  return (
    <div className="p-6 space-y-5 max-w-3xl" style={{ color: 'var(--text-primary)' }}>
      ...
      <PageHeader
        ...
        title="Documents"
        subtitle="Upload a receipt, invoice, or bill — AI extracts the data and creates the accounting entry."
      />
      ...
      {!file && (
        <div
          ...
          style={{
            ...
            boxShadow: dragOver ? '0 0 30px rgba(37,99,235,0.12)' : 'none',
          }}
        >
      ...
      {file && !result && (
        <div className="rounded-xl p-4 space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
          ...
          <button onClick={processFile} disabled={processing}
            ...
            style={{ background: 'var(--accent)', color: '#fff', opacity: processing ? 0.7 : 1 }}>
      ...
      {/* Info card */}
      <div className="rounded-xl p-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderLeft: '3px solid var(--accent)' }}>
        <p className="text-xs font-semibold mb-2" style={{ color: 'var(--accent)' }}>AI Document Intelligence</p>

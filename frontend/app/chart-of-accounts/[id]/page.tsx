    ...
    <div className="min-h-screen p-8 space-y-6" style={{ color: 'var(--text-primary)' }}>
      ...
      {account && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Current Balance', value: fmtBalance(account.current_balance || 0), accent: 'var(--neon-cyan)' },
            { label: 'Total Debits', value: fmt(lines.reduce((s, l) => s + l.debit, 0)) || '$0.00', accent: 'var(--neon-emerald)' },
            { label: 'Total Credits', value: fmt(lines.reduce((s, l) => s + l.credit, 0)) || '$0.00', accent: 'var(--neon-fuchsia)' },
          ].map(card => (
            ...
            <tbody>
              {lines.map(line => (
                <tr key={line.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td className="px-4 py-3 text-right font-medium" style={{ color: line.credit > 0 ? 'var(--neon-fuchsia)' : 'var(--text-muted)' }}>
                    {fmt(line.credit) || '—'}
                  </td>

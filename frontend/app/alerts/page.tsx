...
function AlertCard({ alert, onAction, isDismissed }: { alert: Alert; onAction: () => void; isDismissed?: boolean }) {
  ...
  const color = isDismissed ? 'var(--border-color)' : (SEVERITY_COLORS[alert.severity] || 'var(--neon-cyan)')
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
            Proactive GL intelligence — duplicates, anomalies, AR reminders
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {(critical > 0 || warning > 0) && (
            <div className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium"
              style={{ background: critical > 0 ? 'var(--neon-fuchsia)22' : '#f59e0b22', color: critical > 0 ? 'var(--neon-fuchsia)' : '#f59e0b' }}>
      ...
      <div className="flex gap-2 mb-4">
        {(['open', 'dismissed'] as const).map(f => (
          <button
            ...
            style={{
              background: filter === f ? 'var(--neon-cyan)22' : 'var(--bg-card)',

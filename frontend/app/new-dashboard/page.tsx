  ...
  return (
    <div className="p-6 space-y-4 min-h-screen" style={{ color: 'var(--text-primary)', opacity: transitioning ? 0.6 : 1, transition: 'opacity 0.2s ease' }}>
      ...
      <PageHeader
        eyebrow="Overview"
        title="Dashboard"
        subtitle={<span className="num">{fmtDate(data.period.start)} — {fmtDate(data.period.end)}</span>}
        actions={
          <>
            <button onClick={() => setWidgetModalOpen(true)} className="btn">
              <Plus className="w-3.5 h-3.5" /> Add widget
            </button>
            <div
              className="inline-flex items-center rounded-md"
              style={{ background: 'var(--bg-muted)', border: '1px solid var(--border-color)', padding: 2, height: 30 }}
            >
              {PERIODS.map((p) => {

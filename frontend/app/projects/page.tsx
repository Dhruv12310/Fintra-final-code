      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="rounded px-2 py-1.5" style={{ background: 'var(--bg-secondary)' }}>
          ...
          <p className="text-sm font-semibold"
            style={{ color: overUnder >= 0 ? 'var(--neon-emerald)' : 'var(--accent)' }}>
            {$(Math.abs(overUnder))}
          </p>
        </div>
      </div>
...
export default function ProjectsPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <PageHeader
        title="Projects"
        subtitle="WIP schedules, retention tracking, and job-cost rollups"
        actions={<button onClick={() => setShowNew(true)} className="btn btn-primary"><Plus size={14} /> New Project</button>}
      />

      {!isConstruction && (
        <div className="mb-4 flex items-center gap-2 px-4 py-3 rounded-lg text-sm"
          style={{ background: 'var(--accent-subtle)', border: '1px solid var(--accent)33', color: 'var(--accent)' }}>
          <TrendingUp size={14} />
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-3" style={{ color: 'var(--text-muted)' }}>
          ...
          <button onClick={() => setShowNew(true)}
            ...
            style={{ background: 'var(--accent-subtle)', color: 'var(--accent)', border: '1px solid var(--accent)33' }}>
